...
use crate::{
    ...
    types::{
        ...
        GetAddress as _, GetValue as _, Header, InPoint, M6id, MerkleRoot,
        OutPoint, OutPointKey, SpentOutput, Transaction, VERSION, Version,
...
impl State {
    pub const NUM_DBS: u32 = bitnames::Dbs::NUM_DBS + 11;
    ...
    pub fn validate_batch_icann(
        ...
        if let Some(batch_icann_data) = tx.batch_icann_data() {
            ...
            let Some(verifying_key) = &*constants::BATCH_ICANN_VERIFYING_KEY
            else {
    ...
    pub fn apply_block(
        ...
        block::apply_block(batch_verification_ctxt, self, rwtxn, header, body)
    }
...
mod test {
    ...
    use crate::{
        ...
        types::{
            Address, AuthorizedTransaction, BitName, BlockIndexEvents,
            FilledOutput, FilledOutputContent, FilledTransaction, Hash,
            InPoint, M6id, MutableBitNameData, OutPoint, OutPointKey, Output,

...
use sneed::{DatabaseUnique, DbError, Env, EnvError, RwTxn, UnitKey};
...
impl Net {
    ...
    pub fn new(
        ...
        match version.try_get(&rwtxn, &())? {
            ...
            {
                // types for `known_peers` db changed in v0.17.5
        ...
        let dial_known_peers_handle = {
            ...
            for peer_addr in known_peers {
                ...
                join_map.spawn_on(
                    peer_addr.clone(),
                    async move {
                        net.dial_known_peer(env, peer_addr).await.inspect_err(
                            |err| {
                                tracing::error!(
                                    message = %ErrorChain::new(err)
                                )
                            },
                        )
                    },

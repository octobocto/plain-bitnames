...
use sneed::{DatabaseUnique, DbError, EnvError, RoTxn, RwTxn, UnitKey};
...
const ALPHANET_SEED_PEER_ADDRS: &[PeerAddress<&'static str>] = {
    ...
    const ALPHANET_SERVER: PeerAddress<&'static str> = PeerAddress {
...
impl Net {
    ...
    async fn dial_known_peers(
        ...
        for peer_addr in peer_addrs {
            match self
                .dial_known_peer(sneed::Env::clone(env), peer_addr.clone())
                .await
            {
                Ok(true) => dialed += 1,
                Ok(false) => (),
    ...
    pub fn push_tx(
        ...
        for (address, peer) in peers.iter() {
            if target.is_some_and(|target| target != *address)
            ...
            peer.internal_message_tx
                ...
                .map_err(|err| Error::SendTransaction {
                    address: *address,
                    source: err.into_send_error(),
                })?;
            peer_count += 1;
        }
        Ok(peer_count)
...
mod test {
    ...
    -> anyhow::Result<()> {
        ...
        let addr = SocketAddr::from((Ipv6Addr::LOCALHOST, 4002));
        let resolved = ResolvedPeerAddress::Domain {
            domain: "localhost".to_owned(),
            port: addr.port(),
            addrs: nonempty::NonEmpty {
    ...
    async fn connect_peer_returns_other_quinn_errors() -> anyhow::Result<()> {
        ...
        net.server.close(0_u32.into(), b"test complete");
        let addr = SocketAddr::from((Ipv4Addr::LOCALHOST, 4002));

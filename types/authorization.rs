...
pub fn verify_authorized_transaction(
    ...
    // A BitName reservation can have no inputs, and a frost batch rejects an
    // empty batch.
...
mod tests {
    use super::{
        ...
        SigningKey, authorize, get_address, sign, sign_tx, verify,
        verify_authorizations, verify_authorized_transaction,
    };
    use crate::{
        Address, AuthorizedTransaction, Body, Coinbase, OutPoint, Transaction,
        Txid, VerifyingKey,
    };
    ...
    fn authorized(
        ...
        authorize(rand::rng(), &[(address, key)], transaction)
            .expect("authorize")
    }
    ...
    fn an_address_comes_from_the_compressed_ristretto_point() {
        ...
        let compressed = verifying_key.0.to_element().compress();
        let mut hasher = blake3::Hasher::new();
        let mut reader = hasher.update(compressed.as_bytes()).finalize_xof();
        let mut expected = [0u8; 20];
        reader.fill(&mut expected);
        assert_eq!(get_address(&verifying_key), Address(expected));
    }

    #[test]
    fn a_signature_round_trips_through_hex() {
        let signature =
            sign(rand::rng(), &signing_key(2), Dst::Arbitrary, b"hello");
    ...
    fn a_good_signature_verifies() {
        let ctxt = BatchVerificationContext::new(&mut rand::rng());
        let authorized = authorized(&signing_key(3), one_input_tx(1));
    ...
    #[test]
    fn a_body_with_fewer_authorizations_than_threads_verifies() {
        let ctxt = BatchVerificationContext::new(&mut rand::rng());
        let authorized = authorized(&signing_key(4), one_input_tx(1));
        let body = Body::new(vec![authorized], Coinbase::default());
        verify_authorizations(&ctxt, &body)
            .expect("a body with one authorization must verify");
    }

    #[test]
    fn a_signature_from_another_key_fails() {
        let mut rng = rand::rng();
        let victim = signing_key(5);
    ...
    fn a_signature_replayed_onto_another_transaction_fails() {
        ...
        let original = authorized(&key, one_input_tx(1));
        ...
        let body = Body::new(vec![replayed], Coinbase::default());
    ...
    fn authorize_rejects_a_key_that_does_not_match_the_address() {
        ...
        let other = get_address(&VerifyingKey::from(&signing_key(9)));

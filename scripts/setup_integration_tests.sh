...
# --- Binaries under test ---
# Built from the pinned submodule, so it matches the enforcer library bitnames
# compiles against.
...
echo "Building plain_bitnames_app..."
cargo build --manifest-path "$REPO_ROOT/Cargo.toml" --bin plain_bitnames_app
...
BITNAMES_APP='$REPO_ROOT/target/debug/plain_bitnames_app'
EOF
